% script : " ga_seeding_chootinan_nvs20_s3cx "
% 24/07/2018
% Lance Nexec exécutions pour minimiser sol nvs20 en 3 phases :
...
fich_res = ['Nres_seedingChootinan_', problem, '_s3cx_pop', num2str(Npop), ...
...
tol_eq = 1.0e-6; % tol sur les contraintes d'égalité
...
for iexec = 1:Nexec
    ...
    x_ga(:,1:5) = int32(x_ga(:,1:5)); % fixed integers
...
%     [vu_ga, va_ga, pj_ga, sigmaem_ga, ech_ga, kf_ga, be_ga, p_ga, c_ga] = mapse_3val_sd2(x_ga);
    ...
    options_sqp.TypicalX = ones(1,16);
...
%     [vu_sqp, va_sqp, pj_sqp, sigmaem_sqp, ech_sqp, kf_sqp, be_sqp, p_sqp, c_sqp] = mapse_3val_sd2(x_sqp);
    ...
    ec_p = 0;% écart absolu en p (montre que sqp ne fait qu'arrondir p)
...
val_param_ga = [options_ga.TolFun; options_ga.TolCon; Npop; ...
    options_ga.Generations; options_ga.StallGenLimit];
